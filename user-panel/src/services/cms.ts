...
class CMSService {
  ...
  async getPageContent(pageName: string): Promise<CMSPageData> {
    try {
      const response = await fetch(`${this.apiBase}/api/cms/pages/${pageName}`)
      if (!response.ok) {
  ...
  async getSections(pageName: string): Promise<CMSSection[]> {
    try {
      const response = await fetch(`${this.apiBase}/api/cms/sections/${pageName}`)
      if (!response.ok) {
  ...
  async getAllPages(): Promise<CMSPage[]> {
    try {
      const response = await fetch(`${this.apiBase}/api/cms/pages`)
      if (!response.ok) {
  ...
  async getSettings(): Promise<Record<string, any>> {
    try {
      const response = await fetch(`${this.apiBase}/api/cms/settings`)
      if (!response.ok) {
